    ...
    >
      <a-spin :spinning="formItem.loading || false">
        <a-form-item
          ...
          :name="formItem.field"
          v-bind="{ ...formItem.itemProps, ...validateInfos[formItem.field] }"
        >
          <component
...
import { isString, isFunction, isAsyncFunction } from "./is";
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    let formModel = props.formSchema.formItem.reduce(
    ...
    let rulesRef = props.formSchema.formItem
      .filter((item) => !item.hidden)
      .reduce((previousValue, currentValue) => {
        // if (isReactive(currentValue)) {
        //   currentValue = toRaw(currentValue);
        // }
        currentValue.rules &&
    ...
    // );
    // 如果有默认值，则覆盖
    props.fields && Object.assign(state.formModel, props.fields);
    // 异步设置默认数据
    props.formSchema.formItem.forEach(async (item) => {
      ...
      if (isFunction(item.asyncOptions) || isAsyncFunction(item.asyncOptions)) {
        ...
        // 异步默认值
        state.formModel[item.field] = await item
          .asyncValue(item, proxy)
          .finally(() => (item.loading = false));
      }
    });
    ...
    // console.log(state.formModel, "表单");
    // console.log(state.rulesRef, "表单验证规则");
    ...
    watch(
      ...
      (newVal) => {
        // if (newVal) {
        try {
          Object.keys(newVal).forEach((s) => {
            state.formModel[s] = newVal[s];
          });
        } catch (error) {}
      },
      { deep: true }
