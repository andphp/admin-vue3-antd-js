...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const state = reactive({
      modelRef: formModel,
      rulesRef: props.formSchema.formItem
    ...
    // );
    // 如果有默认值，则覆盖
    props.fields && Object.assign(state.modelRef, props.fields);
    // 异步设置默认数据
    props.formSchema.formItem.forEach(async (item) => {
      ...
      if (isFunction(item.asyncOptions) || isAsyncFunction(item.asyncOptions)) {
        ...
        // 异步默认值
        state.modelRef[item.field] = await item
