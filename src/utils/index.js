...
export function humpToUnderlineWidthObject(data) {
  try {
    if (isObject(data)) {
      let tempData = {};
      Object.keys(data).forEach((v) => {
        let childItem = data[v];
        let key = humpToUnderline(v);
        if (Array.isArray(childItem) && childItem.length > 0) {
          tempData[key] = [];
          childItem.forEach((s) => {
            tempData[key].push(humpToUnderlineWidthObject(s));
          });
        } else {
          tempData[key] = childItem;
        }
      });
      return tempData;
    } else {
