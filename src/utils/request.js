...
import { humpToUnderline, humpToUnderlineWidthObject } from "@/utils/index";
...
instance.interceptors.request.use(
  (config) => {
    ...
    if (config.params) {
      ...
      tempData = humpToUnderlineWidthObject(config.params);
      if (tempData) {
    ...
    if (config.data) {
      ...
      // 驼峰转下划线,递归遍历子属性为数组
      let tempData = undefined;
      tempData = humpToUnderlineWidthObject(config.data);
      if (tempData) {
        config.data = tempData;
      }
    }
